...
Citizen.CreateThread(function()
	local blip = AddBlipforCoord(Config.GaragePos.x, Config.GaragePos.y, Config.GaragePos.z)
...
Citizen.CreateThread(function()
	while true do
		...
		if isInMarker and not HasAlreadyEnteredMarker then
			...
			TriggerEvent('esx_jb_eden_garage2:hasEnteredMarker', currentZone)
		end

		if not isInMarker and HasAlreadyEnteredMarker then
			HasAlreadyEnteredMarker = false
			TriggerEvent('esx_jb_eden_garage2:hasExitedMarker', currentZone)
		end
	end
end)	
-- Fin controle touche
