...
RegisterServerEvent('esx_jb_eden_garage2:setVehicleInGarage')
AddEventHandler('esx_jb_eden_garage2:setVehicleInGarage', function(vehicle, zone)
	local _source = source
	local xPlayer = ESX.GetPlayerFromId(_source)
	local zoneId = zone
	local vehProps = json.decode(vehicle)
	if zoneId == 'Strefa1' then
		MySQL.Async.execute('INSERT INTO parked_vehicles (owner, zone, vehicle) VALUES (@owner, @zone, @vehicle)',
		{
			['@owner']   = xPlayer.identifier,
			['@zone']    = zoneId,
			['@vehicle'] = json.encode(vehProps)
		}, function(rowsChanged)
			TriggerClientEvent('esx:showNotification', _source, 'Twoje pojazd zostało zaparkowane w ~y~garazu~s~')
		end)
	else
		TriggerClientEvent('esx:showNotification', _source, 'Nie mozesz zaparkowac pojazdu w tej strefie')
	end
end)

RegisterServerEvent('esx_jb_eden_garage2:getVehiclesInGarage')
AddEventHandler('esx_jb_eden_garage2:getVehiclesInGarage', function(zone)
	local _source = source
	local xPlayer = ESX.GetPlayerFromId(_source)
	local zoneId = zone
	if zoneId == 'Strefa1' then
		MySQL.Async.fetchAll('SELECT * FROM parked_vehicles WHERE owner = @owner AND zone = @zone',
		{
			['@owner'] = xPlayer.identifier,
			['@zone']  = zoneId
		}, function(result)
			TriggerClientEvent('esx_jb_eden_garage2:setVehiclesInGarage', _source, result)
		end)
	else
		TriggerClientEvent('esx:showNotification', _source, 'Tego pojazdu nie mozesz odebrać z tej strefy')
	end
end)

RegisterServerEvent('esx_jb_eden_garage2:deleteVehicle')
AddEventHandler('esx_jb_eden_garage2:deleteVehicle', function(plate, zoneId)
	local _source = source
	local xPlayer = ESX.GetPlayerFromId(_source)
	if zoneId == 'Strefa1' then
		MySQL.Async.execute('DELETE FROM parked_vehicles WHERE owner = @owner AND plate = @plate',
		{
			['@owner'] = xPlayer.identifier,
			['@plate'] = plate
		}, function(rowsChanged)
			TriggerClientEvent('esx:showNotification', _source, 'Twoje pojazd zostało wyjechane z ~y~garazu~s~')
		end)
	else
		TriggerClientEvent('esx:showNotification', _source, 'Tego pojazdu nie mozesz odebrać z tej strefy')
	end
