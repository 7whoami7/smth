	...
	if not isUsing then
		if _type == 'bsmall' then
			if health < 200 then 
				...
				exports['213RP']:DrawProcent(150, function()
					Citizen.InvokeNative(0x6B76DC1F3AE6E6A3, playerPed, newHealth)
		...
		elseif _type == 'bmedium' then
			...
			exports['213RP']:DrawProcent(300, function()
				FreezeEntityPosition(playerPed, false)
				ClearPedTasks(playerPed)
				isUsing = false
				Citizen.InvokeNative(0x6B76DC1F3AE6E6A3, playerPed, maxHealth)
...
function RemoveItemsAfterRPDeath()
	CreateThread(function()
		...
		ESX.TriggerServerCallback('esx_ambulancejob:removeItemsAfterRPDeath', function()			
			...
			DoScreenFadeIn(800)
			TriggerServerEvent('213rp_logs:triggerLog', "Przeteleportował się na szpital: "..hospital, 'teleportszpital')
		end)
	end)
end
	...
	CreateThread(function()
		...
		while firstScreen do
			...
			if obezwladniony then
				...
				AddTextComponentSubstringPlayerName('~w~Pomoc dostępna za ~m~'..minutes..' minut '..seconds..' sekund')
		...
		while IsDead do
			...
			if obezwladniony then 
				...
				BeginTextCommandDisplayText("STRING")
				AddTextComponentSubstringPlayerName('Przytrzymaj ~m~E~s~ aby tepnąć się na szpital')
					...
					if GetGameTimer() - pressStart > 3000 then
						ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'rp_dead', {
							...
							elements = {
								...
								{ label = ('Greenzone Dino'), value = 'dino'},
								{ label = ('Greenzone Doki'), value = 'doki'}
							}
						}, function (data, menu)
							choosedHospital = data.current.value
							RemoveItemsAfterRPDeath()
							menu.close()
						end, function (data, menu)
							menu.close()
	...
	CreateThread(function ()
		...
		if weapon == `WEAPON_UNARMED` or ((weapon == `WEAPON_RUN_OVER_BY_CAR` or damagedbycar) and sourceofdeath ~= playerPed) or weapon == `WEAPON_NIGHTSTICK` then
			...
			CreateThread(function() 
				...
				exports["213rp_taskbar"]:taskBar(knockoutDuration, "Odzyskiwanie sił...", false, function(cb) 
				end)
			end)
		end
			...
			ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle_interaction',
			{
				...
				if not IsAnyVehicleNearPoint(coords.x, coords.y, coords.z, 5.0) then
					...
					elseif data.current.value == 'hijack' then
						if(not IsPedInAnyVehicle(playerPed)) then
							TriggerServerEvent('213rp:pay', 1500)
							menu.close()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'cloakroom',
	{
		...
		if data.current.value == 'ambulance_wear' then
			...
			if PlayerData.job.name == 'offambulance' then
				TriggerServerEvent('213rp:setJob', 'ambulance', true)
		...
		if data.current.value == 'citizen_wear' then
			...
			if PlayerData.job.name == 'ambulance' then
				...
				TriggerServerEvent('213rp:setJob', 'ambulance', false)
			end
			ESX.TriggerServerCallback('esx_ambulancejob:getPlayerSkin', function(skin, jobSkin)
...
AddEventHandler('hypex_ambulancejob:hypexrevive', function(notBlock)
	...
	if IsDead and not notBlock then
		TriggerServerEvent('213rp_wypadanie:bron')
...
function OpenInventoryMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'items', {
		...
		if data.current.value == 'get' then
			TriggerEvent('213rp:getInventoryItem', 'society_ambulance')
		elseif data.current.value == 'put' then
			TriggerEvent('213rp:putInventoryItem', 'society_ambulance')
		end
	end, function(data, menu)
		menu.close()
		CurrentAction		= 'items_menu'
...
function OpenInventoryMenu2()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'items', {
		...
		menu.close()
		if data.current.value == 'get' then
			TriggerEvent('213rp:getInventoryItem', 'society_sams2')
		elseif data.current.value == 'put' then
			TriggerEvent('213rp:putInventoryItem', 'society_sams2')
		end
	end, function(data, menu)
		menu.close()
		CurrentAction		= 'items2_menu'
