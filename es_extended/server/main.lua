...
AddEventHandler('esx:gitestveInventoryItem', function(target, type, itemName, itemCount)
	...
	elseif type == 'item_account' then
		...
		--	sendToDiscordPieniadze('Przekazywanie | Pieniądze', '**'..sourceXPlayer.name..'['..playerId..']** przekazał **'..targetXPlayer.name..'['..target..']**\n**Typ gotówki: **'..Config.Accounts[itemName]..'\n**Ilość: **'..ESX.Math.GroupDigits(itemCount)..'\n**Czas: **'..czas,56108)
		else
	...
	elseif type == 'item_weapon' then
		if sourceXPlayer.hasWeapon(itemName) then
			...
			sourceXPlayer.removeWeapon(itemName, itemCount[1])
			targetXPlayer.addWeapon(itemName, itemCount[1], itemCount[2], itemCount[3])

			if weaponObject.ammo and itemCount[3] > 0 then
				...
				TriggerClientEvent('sendProximityMessageDo', -1, playerId, playerId, "przekazał broń")
			--	sendToDiscordBron('Przekazywanie | Broń', '**'..sourceXPlayer.name..'['..playerId..']** przekazał **'..targetXPlayer.name..'['..target..']**\n**Rodzaj broni: **'..weaponLabel..'\n**Czas: **'..czas,2061822)
			else
				sourceXPlayer.showNotification(_U('gave_weapon', weaponLabel, targetXPlayer.source))
				targetXPlayer.showNotification(_U('received_weapon', weaponLabel, sourceXPlayer.source))
			end
			...
			if targetXPlayer.hasWeapon(itemName) then
				...
				if weaponObject.ammo then
					...
					if weapon.ammo >= itemCount then
						sourceXPlayer.removeWeaponAmmo(itemName, itemCount[1])
						targetXPlayer.addWeaponAmmo(itemName, itemCount[1], itemCount[2], itemCount[3])

						TriggerClientEvent('sendProximityMessageDo', -1, playerId, playerId, "Obywatel: przekazał ["..ammoLabel.."] do broni "..weapon.label.."")
						...
						sendToDiscordAmmo('Przekazywanie | Amunicja', '**'..sourceXPlayer.name..'['..playerId..']** przekazał **'..targetXPlayer.name..'['..target..']**\n**Do broni: **'..weapon.label..'\n**Czas: **'..czas,8421504)
					end
				end
			else
...
AddEventHandler('esx:removeInventoryItem', function(type, itemName, itemCount)
	...
	if type == 'item_standard' then
		if itemCount == nil or itemCount < 1 then
			...
			--	sendToDiscordItemyW('Wyrzucanie | Itemy', '**'..xPlayer.name..'['..source..']** wyrzucił przedmiot\n**Nazwa przedmiotu: **'..xItem.label..'\n**Ilość: **'..itemCount..'\n**Czas: **'..czas,16711680)
			end
		end
	elseif type == 'item_account' then
		if itemCount == nil or itemCount < 1 then
			...
			--	sendToDiscordPieniadzeW('Wyrzucanie | Pieniądze', '**'..xPlayer.name..'['..source..']** wyrzucił pieniądze \n**Typ gotówki: **'..string.lower(account.label)..'\n**Ilość: **'..ESX.Math.GroupDigits(itemCount)..'\n**Czas: **'..czas,56108)
			end
		end
	elseif type == 'item_weapon' then
		...
		if xPlayer.hasWeapon(itemName) then
			...
			if weaponObject.ammo and weapon.ammo > 0 then
				...
				TriggerClientEvent('sendProximityMessageDo', -1, playerId, playerId, "upuścił na ziemię broń")
			--	sendToDiscordBronW('Wyrzucanie | Broń', '**'..xPlayer.name..'['..source..']** wyrzucił broń \n**Rodzaj broni: **'..weapon.label..'\n**Czas: **'..czas,2061822)
			else
				xPlayer.showNotification(_U('threw_weapon', weapon.label))
				TriggerClientEvent('sendProximityMessageDo', -1, playerId, playerId, "upuścił na ziemię broń")
			--	sendToDiscordBronW('Wyrzucanie | Broń', '**'..xPlayer.name..'['..source..']** wyrzucił broń \n**Rodzaj broni: **'..weapon.label..'\n**Czas: **'..czas,2061822)
