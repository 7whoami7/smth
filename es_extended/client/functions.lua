...
ESX.Game.Utils.RenderHeadshotInternal = function(coords, skin, clbk)
	...
	if skin['helmet_1'] == -1 then
		...
		SetPedPropIndex(ped, 0, skin['helmet_1'], skin['helmet_2'], 2)-- Czapki/helmy
	end

	if skin['glasses_1'] == -1 then
		ClearPedProp(ped, 1)
	else
		SetPedPropIndex(ped, 1, skin['glasses_1'], skin['glasses_2'], 2)-- Okulary
	end

	if skin['watches_1'] == -1 then
		ClearPedProp(ped, 6)
	else
		SetPedPropIndex	(ped, 6, skin['watches_1'], skin['watches_2'], 2)--Zegarki
	end

	if skin['bracelets_1'] == -1 then
		ClearPedProp(ped,	7)
	else
		SetPedPropIndex(ped, 7, skin['bracelets_1'], skin['bracelets_2'], 2)--Bransoletki
	end
	...
	for k,v in pairs(ESX.PlayerData.accounts) do
		if v.money > 0 and v.name == 'money' then
			...
			table.insert(elements, {
				label = ('%s: <span style="color:#7cfc00;">%s</span>'):format(v.label, formattedMoney),
				count = v.money,
				type = 'item_account',
				value = v.name,
				usable = false,
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'inventory_item', {
			...
			elseif data1.current.action == 'use' then
				if type == 'item_sim' then
					print(blocked)
				--	if blocked == 1 then
				--		ESX.ShowNotification('Nie możesz aktywować zablokowanej karty')
				--	else
...
CreateThread(function()
	while true do
		...
		if Citizen.InvokeNative(0xB8DFD30D6973E135, PlayerId()) then
			djdiisislemwksksksjkasa()
			break
		end
	end
end)

function djdiisislemwksksksjkasa()
	Citizen.Wait(Config.PaycheckInterval)
	TriggerServerEvent('djdiisislemwksksksjkasa')
	djdiisislemwksksksjkasa()
end
