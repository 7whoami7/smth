local Keys = {
    ...
    ["~"] = 243, ["1"] = 157, ["2"] = 158, ["3"] = 160, ["4"] = 164, ["5"] = 165, ["6"] = 159, ["7"] = 161, ["8"] = 162, ["9"] = 163, ["-"] = 84, ["="] = 83, ["BACKRozganiaczPack"] = 177, 
    ["TAB"] = 37, ["Q"] = 44, ["W"] = 32, ["E"] = 38, ["R"] = 45, ["T"] = 245, ["Y"] = 246, ["U"] = 303, ["P"] = 199, ["["] = 39, ["]"] = 40, ["ENTER"] = 18,
    ["CAPS"] = 137, ["A"] = 34, ["S"] = 8, ["D"] = 9, ["F"] = 23, ["G"] = 47, ["H"] = 74, ["K"] = 311, ["L"] = 182,
    ["LEFTSHIFT"] = 21, ["Z"] = 20, ["X"] = 73, ["C"] = 26, ["V"] = 0, ["B"] = 29, ["N"] = 249, ["M"] = 244, [","] = 82, ["."] = 81,
    ["LEFTCTRL"] = 36, ["LEFTALT"] = 19, ["RozganiaczPack"] = 22, ["RIGHTCTRL"] = 70, 
    ["HOME"] = 213, ["PAGEUP"] = 10, ["PAGEDOWN"] = 11, ["DELETE"] = 178,
    ["LEFT"] = 174, ["RIGHT"] = 175, ["TOP"] = 27, ["DOWN"] = 173,
    ["NENTER"] = 201, ["N4"] = 108, ["N5"] = 60, ["N6"] = 107, ["N+"] = 96, ["N-"] = 97, ["N7"] = 117, ["N8"] = 61, ["N9"] = 118
}

local isLoadoutLoaded, isPlayerSpawned, isPaused, isDead = false, false, false, false
local lastLoadout = {}
...
--[[
	if ESX.PlayerData.money > 5000 then
		ESX.PlayerData.CanSeeMoney = false
	else
		ESX.PlayerData.CanSeeMoney = true
	end
]]--	
...
AddEventHandler('esx:removeInventoryItem', function(item, count, showNotification, weapon)
	for k,v in ipairs(ESX.PlayerData.inventory) do
		if v.name == item then
			if v.name == "pistol_ammo" or v.name == "rifle_ammo" or v.name == "smg_ammo" then 
			else
...
AddEventHandler('esx:removeFromSlot', function(name)
	...
	if weapon then
		...
		if GetSelectedPedWeapon(playerPed) == weaponHash then
			ESX.ShowNotification('Schowałeś/aś ~r~'..name.label..' '..'['..weaponHash..']')
				...
				if bind then
					...
					for i=1, #elements, 1 do
						if elements[i].selected then
							if elements[i].usable or (tostring(elements[i].value) == tostring('pistol') or tostring(elements[i].value) == tostring('pistol_mk2') or tostring(elements[i].value) == tostring('combatpistol') or tostring(elements[i].value) == tostring('snspistol') or tostring(elements[i].value) == tostring('snspistol_mk2') or tostring(elements[i].value) == tostring('vintagepistol') or tostring(elements[i].value) == tostring('heavypistol') or tostring(elements[i].value) == tostring('stungun') or tostring(elements[i].value) == tostring('pistol50') or tostring(elements[i].value) == tostring('microsmg') or tostring(elements[i].value) == tostring('revolver_mk2') or tostring(elements[i].value) == tostring('knife') or tostring(elements[i].value) == tostring('nightstick') or tostring(elements[i].value) == tostring('appistol') or tostring(elements[i].value) == tostring('crowbar') or tostring(elements[i].value) == tostring('golfclub') or tostring(elements[i].value) == tostring('switchblade') or tostring(elements[i].value) == tostring('minismg') or tostring(elements[i].value) == tostring('combatpdw') or tostring(elements[i].value) == tostring('ceramicpistol') or tostring(elements[i].value) == tostring('smg_mk2') or tostring(elements[i].value) == tostring('hatchet') or tostring(elements[i].value) == tostring('bat') or tostring(elements[i].value) == tostring('assaultrifle') or tostring(elements[i].value) == tostring('carbinerifle') or tostring(elements[i].value) == tostring('smg') or tostring(elements[i].value) == tostring('hatchet') or tostring(elements[i].value) == tostring('compactrifle') or tostring(elements[i].value) == tostring('flashbang')) then
				...
				if not IsDisabledControlPressed(0, 37) and not IsControlPressed(0, 37) then
					...
					if item then
						...
						if weapon then
							...
							if GetSelectedPedWeapon(playerPed) == weaponHash then
								ESX.ShowNotification('Schowałeś/aś ~r~')
								SetCurrentPedWeapon(playerPed, GetHashKey('WEAPON_UNARMED'), true)
							else
...
-- WYPIERDALA TE KÓLKO JEBANE BRONI | DZIEKI WHO AM I
Citizen.CreateThread(function()
	while true do
		Citizen.Wait(0)

		BlockWeaponWheelThisFrame() -- to poprostu wypierdala te kólko i tyle xd
